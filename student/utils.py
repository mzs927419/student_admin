...
def checkIdcard(idcard):
    ...
    if (not area[(idcard)[0:2]]):
        print(Errors[4])
        dic.update({'message': Errors[4]})
    dic.update({'city':area[idcard[0:2]]})
    ...
    if (len(idcard) == 15):
        ...
        if (re.match(ereg, idcard)):
            print(Errors[0])
            dic.update({'message': Errors[0]})
        else:
            print(Errors[2])
            dic.update({'message': Errors[2]})
        ...
        if (re.match(ereg, idcard)):
            ...
            if (M == idcard_list[17]):  # 检测ID的校验位
                ...
                if int(idcard[16]) % 2 == 0:
                    ...
                    dic.update({'sex': '男'})
                dic.update({'birthday': idcard[6:10]+'-'+idcard[10:12]+'-'+idcard[12:14]})
                dic.update({'status': True})

            else:
                print(Errors[3])
                dic.update({'message': Errors[3]})
        else:
            print(Errors[2])
            dic.update({'message': Errors[2]})
    else:
        print(Errors[1])
        dic.update({'message': Errors[1]})
    return dic
